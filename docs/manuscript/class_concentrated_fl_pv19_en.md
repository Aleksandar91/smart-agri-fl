...
Cross-silo federated learning is often evaluated with aggregate accuracy and a global non-IID parameter, even though one silo may hold most examples of a particular class. We study whether this class-level ownership concentration changes how targeted label flipping appears in a small-silo visual benchmark. The experiment uses a prospectively locked 19-class PlantVillage subset with physical-leaf grouping, five clients, paired partition seeds, MobileNetV3-Small heads, and a provisioned attacker chosen as the largest owner of each source class. The treatment matrix covers clean FedAvg/FedProx training, full and fractional source-label flipping, standard robust aggregators, a simple reversed-update attack, clean robust-aggregator baselines, and a within-dataset PV-19-full scale extension. The later statistical comparison list was fixed on archived predictions, not as a public preregistration or a documented blind analysis of means.

On the locked test set, stronger heterogeneity reduced FedAvg macro-F1 from about 0.86 under IID partitioning to about 0.59 at Dirichlet α = 0.1. Full attacker-local source flipping caused large source-recall losses while overall accuracy often moved only a few points; at α = 0.1, a 50% flip still produced about 23 percentage points of pair-averaged source harm with about one point of accuracy loss. This visibility gap is largely the arithmetic consequence of a source class occupying only a small share of a 19-class test set, but source-to-target confusion shows that Apple and Cherry errors were mostly redirected to the intended target. Seed-blocked paired tests found positive mean source harm on all five seeds at every α, but with five seed units the exact two-sided test cannot reject at 0.05. Mixed-model and secondary dose analyses associate larger ownership monopoly with greater targeted harm; under the provisioned-owner rule, however, monopoly at full flipping is also the globally corrupted source-class fraction, and across fractions harm tracks \(qM_c\) rather than a separable matched-dose concentration effect. Robust aggregation did not repair the attacked source class at α = 0.1, and clean baselines showed that Krum’s global collapse was mostly a clean non-IID aggregation tax while still leaving attack-induced source harm. The contribution is a reproducible evaluation protocol and evidence that FL reports should include class ownership and class-level harm alongside α and aggregate utility.
...
This paper does not propose a new robust aggregation rule [6], [7], poisoning attack [8], or FL optimiser [11], but it brings a locked evaluation protocol for how class-level ownership interacts with clean utility, targeted label flipping, and standard robust aggregation on identical partitions.
...
7. A locked within-dataset scale extension on PV-19-full (same 19 classes; cap-then-split versus full-then-split) on which the clean heterogeneity/FedProx pattern and the apple-flip visibility gap appear again; it is not an independent replication.
...
The review is organised by function, not by application slogan. Plant-disease and smart-farm FL papers are cited only as the application setting: they show where class-concentrated silos can arise, not a leaderboard this work tries to beat, and not the claim that the contribution is a new crop-disease model.

### 2.1 Measuring statistical heterogeneity and federated optimisation

FedAvg averages client weights, usually weighted by local sample counts [1]. When local objectives differ, more local steps can increase client drift. Li et al. analyse FedAvg under non-identical data and show that heterogeneity slows the convergence rate for strongly convex smooth problems [12]. Hsu, Qi, and Brown synthesise client label vectors from a Dirichlet distribution with concentration α: as α → ∞ clients match a shared prior; as α → 0 each client tends toward a single class. On CIFAR-10 they report that FedAvg test accuracy falls as α decreases [5]. We use a class-wise allocation inspired by Hsu as the heterogeneity axis, not a label mixture drawn inside a client; those α values are not automatically comparable to Hsu’s client-mixture experiments. We add a class-level monopoly statistic on the same partitions rather than treating α as a complete description of ownership.

FedProx adds a proximal term toward the current global weights. Li et al. give convergence guarantees under statistical and systems heterogeneity and report more stable, higher-accuracy behaviour than FedAvg on heterogeneous federated datasets [11]. SCAFFOLD uses control variates to correct client drift [13]. FedBN keeps batch-normalisation statistics local under feature-space non-IID data [14]. Kairouz et al. survey these and other open problems [2]. SCAFFOLD and FedBN are not in the locked matrix. We use FedAvg and FedProx so that later aggregator comparisons are not confounded by an untested optimiser.

### 2.2 Poisoning and robust aggregation

Biggio et al. study data poisoning against support-vector machines in the centralised setting [15]. In FL, Tolpegin et al. implement targeted poisoning as a data-only label flip: each malicious participant rewrites every local example of a source class to a target class, then trains with the honest optimiser. On CIFAR-10 and Fashion-MNIST they show that a small fraction of such participants can cut source-class recall while remaining classes stay comparatively intact, which they treat as a detection-evasion property [9]. LFighter is a later defence for that attack: it extracts output-layer gradients of putative source and target neurons, clusters those features, and filters suspected updates before FedAvg [16]. Our primary attack is the same data-only rewrite, but with a single provisioned attacker — the silo that holds the most source-class training images on that partition — rather than a random malicious fraction.

Attacks that manipulate the uploaded parameter vector are a different threat. Blanchard et al. design Krum for Byzantine gradient vectors [6]. Fang et al. formulate local model poisoning as an optimisation problem against Krum, coordinate median, trimmed mean, and Bulyan, with the goal of raising global test error [8]; El Mhamdi et al. introduce Bulyan as a follow-on aggregation rule that reduces the leeway left by earlier Byzantine-resilient rules [17]. Model-replacement backdoors show a stronger targeted model-poisoning line in which a malicious client scales its update to overwrite part of the global model [18]. Phase 4.2c is much narrower: it simply reverses or half-reverses the honest Apple-monopoly client’s delta, so it is a contrast attack rather than a reimplementation of those optimised model-poisoning methods.

Coordinate-wise median and trimmed mean reduce the influence of coordinate outliers [7]. Krum (and MultiKrum) select updates that are close to a majority of others in Euclidean distance [6]. The difficulty is that distance-based robustness and non-IID local objectives do not automatically align: robust optimisers can fail even without Byzantine clients [19], and bucketing was proposed specifically to adapt Byzantine-robust learning to heterogeneous datasets [20]. Shejwalkar et al. further argue that FL poisoning studies should spell out the threat model and evaluate attacks under production-realistic constraints [21]. FoolsGold adapts client learning rates from historical cosine similarity of updates, targeting Sybil clones that share a poisoning objective [22]. We cite FoolsGold and LFighter as related defences, not as methods we re-implemented in phases 3–4.2a. Our 4.2a slice uses median, trimmed mean, Krum, and MultiKrum only.

### 2.3 Evaluation practice and agricultural FL

LEAF argues that federated experiments should report more than a single average accuracy: the authors include percentile performance across devices, systems cost, and how accuracy is weighted (per device versus per example), and they criticise purely artificial partitions of MNIST or CIFAR as an incomplete model of federated data [23]. Hsu et al. supply a continuous non-IID knob for visual classification [5]. Neither benchmark studies class monopoly as a predictor of targeted label-flip harm, and neither is a plant-disease testbed.

Application papers on crop images typically report overall accuracy after FedAvg or transfer-learning FL. Kabala et al. split four PlantVillage crop subsets (apple, corn, grape, tomato) equally and at random among 3, 5, or 7 clients, train with FedAvg, and vary the number of clients, communication rounds, and local epochs. Among the architectures they compare, ResNet50 reached mean accuracy and F1 near 99.5% on grape leaves in that protocol; they also note that ViT_B16 and ViT_B32 took more compute [3]. Idoje et al. apply FedAvg to a different agricultural task — predicting rice, maize, or chickpea from temperature, humidity, pH, and rainfall — and compare it with centralised Gaussian naïve Bayes on a PySyft testbed [4]. Aggarwal et al. report federated transfer learning and federated feature extraction for four rice-leaf diseases (5,932 images) with EfficientNetB3 as the selected base model at 99% validation accuracy, and they record GPU, CPU, and memory use alongside accuracy, precision, and recall, on both IID and non-IID splits [24]. We cite these papers as the application setting. None of them is a paired clean-versus-targeted-poisoning evaluation on locked partitions; none reports class ownership concentration as a predictor of attack harm; Kabala’s partitions are equal random draws, not Dirichlet. We do not treat their accuracy numbers as baselines to “beat” with a new network.
...
The uncapped leaf-safe 19-class set (`pv19-full-774007483a1d`) has 20,597 images and a locked test of 3,110 images. It is the same 19 classes and the same split seed (20260824) as the capped set. Capped selects at most 300 images per class (`subset_seed` 20260824) and *then* assigns groups to train/validation/test; full assigns splits on the complete group list. Shared leaf groups therefore do not inherit the capped assignment unless that class already had no extra groups (only `Apple___Cedar_apple_rust` and `Potato___healthy`). Phase 5 is a within-dataset scale extension, not a held-out second dataset. Partitions reuse the capped seeds and α values. The apple attacker is recomputed on each full train partition (max `Apple___healthy`, tie lowest index), not copied from the capped attacker table. The locked 45-job slice is clean α = 0.1 FedAvg/FedProx and IID FedAvg; apple flip 1.0 × FedAvg on α = 0.1 / 0.5 / IID; and median under the same flip. There is no paired full clean FedAvg at α = 0.5.
...
The locked-test confusion matrix supplies accuracy, balanced accuracy, macro-F1, per-class recall, worst-class recall, and the 10th-percentile class recall archived with the evaluation output. The tables below headline accuracy, macro-F1, balanced accuracy, worst-class recall, source-class recall, and source-class harm because those quantities answer RQ1–RQ3 directly. Under attack we also report source-class harm (clean minus attacked source recall, paired) and drops in accuracy and macro-F1 versus the paired clean E=1 run. For 4.2a we report source-recall recovery versus attacked FedAvg, and accuracy/macro-F1 relative to attacked FedAvg and to clean FedAvg.
...
**Classic Krum paid the largest gap to clean FedAvg** and did not restore source recall on average. Its attacked accuracy was about 0.30 at α = 0.1 and 0.53 at α = 0.5, corresponding to 33- and 28-point gaps to the paired clean FedAvg baseline. With five clients and f = 1, Krum keeps a single update; under class-concentrated non-IID that update is a poor global model. This matches the known tension between distance-based robustness and heterogeneous local objectives [19], [20]. The 0.511 source-harm column versus clean FedAvg at α = 0.1 is not all poison: Table 7 splits it into 0.262 clean source-class cost and 0.250 attack-induced harm versus clean Krum.

**Median and trimmed mean stayed close to attacked FedAvg overall accuracy** at α = 0.5 and IID (gaps of about 0–2 points in the pair-averaged table) and showed modest mean source-recall recovery versus attacked FedAvg at α = 0.5 (~10 points). They did not remove source harm at α = 0.1 (pair-averaged harm still ~0.59–0.63). MultiKrum (keep 4) was usually between keep-one Krum and the coordinate-wise rules on overall accuracy; at α = 0.1 it exceeded median but not trimmed mean. Its mean source recovery versus FedAvg remained small.
...
**Krum’s α = 0.1 overall-score collapse is an aggregator tax; its source-class deficit is not.** Clean Krum accuracy was 0.289 versus 0.304 under flip 1.0. The 33-point 4.2a gap to clean FedAvg was already present without poison. Pair-averaged source recall under clean Krum was 0.564; under flip 1.0 it was 0.314, so attack-induced source harm versus clean Krum is 0.250. The 0.511 deficit versus clean FedAvg therefore splits about evenly into clean source-class cost (0.262) and poison (0.250). Keep-one Krum was scored on the full `state_dict`, including batch-norm running statistics and the integer `num_batches_tracked` counter. That rule has a large clean-utility cost at α = 0.1 (Table 7). Pairwise distances were not decomposed, so we do not claim a mechanism that honest clients look like outliers; the empirical fact is the clean collapse plus residual source harm versus clean Krum.
...
Phase 5 ran the locked 45 jobs on the uncapped 19-class set (test 3,110 images). Table 8 is n = 5 seeds. We did not use Table 8 to choose any later treatment. The slice is a **within-dataset scale extension**, not an independent replication.

**Splits overlap but are not inherited.** All 5,526 capped paths and all 993 capped leaf groups appear in full. Of those images, 2,166 (397 groups) change split. Of the 850 capped-test images, 216 remain in the full test; 504 land in full train and 130 in full validation. Only two classes already at or below the 300-image cap (`Apple___Cedar_apple_rust`, `Potato___healthy`) keep an identical group set and therefore the same split assignment, including the same 24 Potato test images. Apple healthy, the phase-5 source class, keeps 8 of 50 capped-test images in the full test.

**Clean accuracy rose, but train size and test composition both changed.** At α = 0.1 FedAvg, full accuracy / balanced accuracy / macro-F1 were 0.759 / 0.729 / 0.699 against capped 0.636 / 0.643 / 0.591. Under IID the same trio was 0.900 / 0.886 / 0.887 versus capped 0.861 / 0.862 / 0.861. The capped test is nearly even (support 24–53); the full test is not (24–320, Tomato bacterial spot). Source-class test shares \(\pi_s\) are Apple 50/850 = 0.059 versus 247/3110 = 0.079, Cherry 46/850 = 0.054 versus 129/3110 = 0.041, and Potato 24/850 = 0.028 versus 24/3110 = 0.008. FedProx again helped at α = 0.1 (accuracy 0.794 vs 0.759). The heterogeneity gap remains; we do not attribute the higher full accuracy solely to more training images (3,797 → 14,372).

**The apple visibility gap appeared again on this larger test, not on a shared hold-out.** At α = 0.1, FedAvg accuracy dropped 3.7 points versus the paired full clean run, while source-class recall harm was 58.8 points. Mean source recall was 0.264 on both variants. That three-decimal match is an archive fact, not independent-replication evidence: per-seed recalls were 0 / 0.88 / 0.44 / 0 / 0 (capped) and 0 / 0.915 / 0.405 / 0 / 0 (full); three of five seeds collapse to zero in both. Under IID the accuracy drop was 0.6 points and source harm 10.0 points (capped apple harm 0.100).
...
**Table 8.** PV-19-full locked test (mean, n = 5). Within-dataset scale extension, not an independent replication. Apple pair only. Drop and harm versus paired full clean FedAvg E=1 where that job exists. Median recovery versus full FedAvg flip 1.0. Dashes: no paired full clean at α = 0.5. Clean balanced accuracy and macro-F1 are in the text above.
...
**Table 9.** Locked paired tests on five partition seeds. Mean: unweighted mean of the five seed-level gaps (for F2 and F3.1–F3.4 that is the mean of three locked pairs inside each seed, which equals the 15-job mean). CI: percentile bootstrap 95% interval for that five-seed mean [30]; when all five gaps share a sign this interval does not replace the exact test. p: sign-flip permutation of the five seed means [29]; Holm p: adjustment inside the family [27]. Family IDs F1–F3 are 31 August comparison bundles, not laboratory phase numbers. F1 / F1b / F3.5 use macro-F1; F2a source-recall harm; F2b operational visibility (harm minus accuracy drop); F2c FedProx minus FedAvg harm; F3.1–F3.4 source-recall recovery versus attacked FedAvg. n = 5 cannot give two-sided permutation p below 0.0625.
...
**Table 10.** Protocol linear mixed model [31] of locked-test class recall (2,280 rows; 120 jobs; 90 targeted rows from three source classes). Random intercepts for class (variance 0.012) and seed (8.9×10^{−5}, boundary). Targeting = 1 iff the class is the locked flip source in that job. Entropy is normalised \(\tilde{H}_c\). Coefficients are REML associations, not causal effects.
...
A binomial GEE [32] on the same counts, clustered by the five seeds, keeps the monopoly×targeted sign (logit −3.47). Five clusters are too few for that sandwich estimator to replace Table 10.

**Table 11.** Secondary comparison of α, monopoly, and induced dose \(qM_c\) on archived FedAvg/FedProx jobs. Spearman uses average ranks for ties. Partial Spearman residualises those ranks on α dummies, then takes Pearson of the residuals. LOSO RMSE: fit on four partition seeds, score the fifth, then pool. Not a Holm family; Table 10 is unchanged. IID \(M_c\) range is 0.207–0.231. The \(M_c\) coefficient given α and \(qM_c\) uses OLS default standard errors; that p-value is not a seed-clustered test of leftover monopoly.

| Quantity | n | Result |
|----------|---|--------|
| Spearman \(M_c\) vs harm, FedAvg, flip 1.0 | 45 | \(\rho=0.78\) |
| Same, FedAvg and FedProx | 90 | \(\rho=0.79\) |
| Partial Spearman given α | 90 | \(\rho=0.63\) |
| Within-α Spearman (both algorithms) | 30 | 0.30 / 0.62 / 0.66 at 0.1 / 0.5 / IID |
| OLS \(R^2\), harm ~ α | 90 | 0.42 |
| OLS \(R^2\), harm ~ α + \(M_c\) | 90 | 0.58 |
| LOSO RMSE, harm ~ α | 90 | 0.249 |
| LOSO RMSE, harm ~ α + \(M_c\) | 90 | 0.210 |
| Spearman harm vs \(qM_c\), \(q\in\{0.25,0.50,1.0\}\) | 270 | \(\rho=0.78\) |
| OLS \(R^2\), harm ~ α + \(qM_c\) | 270 | 0.68 |
| LOSO RMSE, harm ~ α + \(qM_c\) | 270 | 0.143 |
| \(M_c\) given α + \(qM_c\) | 270 | −0.014 (OLS default p=0.83); LOSO RMSE 0.144 |
...
**Application framing.** The numbers are laboratory scores on processed PlantVillage leaves [10], not greenhouse or orchard error rates. Domain shift to handheld photographs is a separate validity threat. Section 5.7 shows that enlarging the same 19 classes, with a different test composition, does not remove the visibility gap; it is not a substitute for field photographs.
...
- **Statistical.** n = 5 partition-and-training seeds; several source-recall distributions are heavy-tailed. Exact paired permutation on five seed-level means cannot go below p = 0.0625 two-sided, including F2a/F2b after averaging the three locked pairs within seed. Holm is within family. The mixed model is Gaussian on bounded recall (198 fitted values outside [0, 1]); the seed intercept is on the boundary (class variance 0.012). Monopoly and entropy have Pearson −0.98 (VIF 21). The targeting interaction uses three source classes. Tables 11–12 are secondary. \(M_c\) and \(qM_c\) are collinear at flip 1.0 by construction.
- **External.** Five silos; 19 leaf-safe classes; 128 px MobileNet head. PV-19-full is a within-dataset scale extension of the same 19 classes, not a second dataset; other crops, climate-tabular tasks, or field sets may differ [3], [4], [24].
- **Contamination.** Group-safe split and per-slice scoring of the same 850-image test reduce leakage relative to naive image-level splits. That test was reused across phases, not held until the whole study ended. The PV-19-full test is not that hold-out: 2,166 capped images change split, and only 216 of 850 capped-test images remain in the full test. Residual near-duplicates below the unused perceptual threshold cannot be ruled out to zero.
...
[6] P. Blanchard, E. M. El Mhamdi, R. Guerraoui, and J. Stainer, “Machine learning with adversaries: Byzantine tolerant gradient descent,” in *Proc. NeurIPS*, 2017, pp. 118–128.

[7] D. Yin, Y. Chen, K. Ramchandran, and P. Bartlett, “Byzantine-robust distributed learning: Towards optimal statistical rates,” in *Proc. ICML*, 2018, pp. 5650–5659.

[8] M. Fang, X. Cao, J. Jia, and N. Gong, “Local model poisoning attacks to Byzantine-robust federated learning,” in *Proc. 29th USENIX Security Symp.*, 2020, pp. 1605–1622.

[9] V. Tolpegin, S. Truex, M. E. Gursoy, and L. Liu, “Data poisoning attacks against federated learning systems,” in *ESORICS*, 2020, pp. 480–501.
...
[11] T. Li, A. K. Sahu, M. Zaheer, M. Sanjabi, A. Talwalkar, and V. Smith, “Federated optimization in heterogeneous networks,” in *Proc. MLSys*, 2020.

[12] X. Li, K. Huang, W. Yang, S. Wang, and Z. Zhang, “On the convergence of FedAvg on non-IID data,” in *Proc. ICLR*, 2020.
...
[14] X. Li, M. Jiang, X. Zhang, M. Kamp, and Q. Dou, “FedBN: Federated learning on non-IID features via local batch normalization,” in *Proc. ICLR*, 2021.

[15] B. Biggio, B. Nelson, and P. Laskov, “Poisoning attacks against support vector machines,” in *Proc. ICML*, 2012.

[16] N. M. Jebreel, J. Domingo-Ferrer, D. Sánchez, and A. Blanco-Justicia, “LFighter: Defending against the label-flipping attack in federated learning,” *Neural Networks*, vol. 170, pp. 111–126, 2024.

[17] E. M. El Mhamdi, R. Guerraoui, and S. Rouault, “The hidden vulnerability of distributed learning in Byzantium,” in *Proc. ICML*, 2018, pp. 3521–3530.
...
[19] S. P. Karimireddy, L. He, and M. Jaggi, “Learning from history for Byzantine robust optimization,” in *Proc. ICML*, 2021, pp. 5311–5319.

[20] S. P. Karimireddy, L. He, and M. Jaggi, “Byzantine-robust learning on heterogeneous datasets via bucketing,” in *Proc. ICLR*, 2022.

[21] V. Shejwalkar, A. Houmansadr, P. Kairouz, and D. Ramage, “Back to the drawing board: A critical evaluation of poisoning attacks on production federated learning,” in *Proc. IEEE Symp. Security and Privacy*, 2022, pp. 1354–1371.

[22] C. Fung, C. J. M. Yoon, and I. Beschastnikh, “The limitations of federated learning in Sybil settings,” in *Proc. RAID*, 2020, pp. 301–316.

[23] S. Caldas *et al.*, “LEAF: A benchmark for federated settings,” arXiv:1812.01097, 2018.

[24] M. Aggarwal, V. Khullar, N. Goyal, and T. A. Prola, “Resource-efficient federated learning over IoAT for rice leaf disease classification,” *Comput. Electron. Agric.*, vol. 221, Art. no. 109001, 2024.

[25] A. G. Howard *et al.*, “Searching for MobileNetV3,” in *Proc. ICCV*, 2019, pp. 1314–1324.
...
[28] F. Wilcoxon, “Individual comparisons by ranking methods,” *Biometrics Bull.*, vol. 1, no. 6, pp. 80–83, 1945.

[29] M. D. Ernst, “Permutation methods: A basis for exact inference,” *Statist. Sci.*, vol. 19, no. 4, pp. 676–685, 2004.

[30] B. Efron and R. J. Tibshirani, *An Introduction to the Bootstrap*. New York, NY, USA: Chapman & Hall, 1993.

[31] N. M. Laird and J. H. Ware, “Random-effects models for longitudinal data,” *Biometrics*, vol. 38, no. 4, pp. 963–974, 1982.

[32] K.-Y. Liang and S. L. Zeger, “Longitudinal data analysis using generalized linear models,” *Biometrika*, vol. 73, no. 1, pp. 13–22, 1986.
